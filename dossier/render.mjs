...
await p.goto(pathToFileURL(path.join(here, 'index.html')).href, { waitUntil: 'networkidle' });
...
for (const [i, el] of pages.entries()) await el.screenshot({ path: path.join(os.tmpdir(), `dossier-${i + 1}.png`) });
await b.close();
